...
def make_oneliner(name: str, grade: str | None, score: int) -> str:
    s = score
    if s >= 930: return f"지금 {name}님의 오굿스코어는, {s} 점! 최상위 안정 구간이에요 🔒 지금처럼 결제일에 안정적으로 납부한다면 오굿페이 VVIP!!"
    ...
    if s >= 730: return f"지금 {name}님의 오굿스코어는, {s} 점! 아 낮은 점수 구간이에요. 지출을 조금 줄여볼까요?"
    if s >= 700: return f"지금 {name}님의 오굿스코어는, {s} 점! 조금 더 절약하면 더 좋은 점수가 나올 거예요."
    if s >= 650: return f"지금 {name}님의 오굿스코어는, {s} 점! 정비 타이밍. 지금 이상태라면 위험해요!"
    return         f"지금 {name}님의 오굿스코어는, {s} 점! 이렇게 가다간 신용불량자가 될 수도 있어요. 지금 바로 오굿페이와 상담해요!"
